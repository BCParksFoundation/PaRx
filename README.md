...
The shared script also loads Google's reCAPTCHA API once per page, updates every Salesforce `captcha_settings` timestamp independently, handles form-scoped reCAPTCHA completion and registration-form date formatting, and keeps labels scoped correctly when Salesforce field IDs are repeated across forms. Patient-log fragments intentionally keep their date conversion self-contained so the Salesforce date payload does not depend on shared-script initialization. Do not add duplicate reCAPTCHA API, timestamp, or callback scripts to individual Webflow embeds.
